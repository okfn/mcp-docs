...
That is the strongest field evidence the pilots produced for the
[open model](../overview/open-model.md). The goal was never just to
ship an assistant; it was to leave behind a method other teams can
reuse. Here it happened without us asking.

## Users spot new uses

While testing, government users started seeing other places the same
pattern would fit. One example they raised: answering the kinds of
questions that arrive as formal access-to-information requests, where
today a person looks up the data by hand. None of this was in the
pilot's scope; the point is that once people see the pattern work on
one dataset, they start mapping it onto their own problems.

## It integrated a team

The pilot brought together a government group that had not worked much
together before, around a concrete common goal. By the end, the
collaboration itself was named as one of the most valuable outcomes:
different teams, one objective, and a working result to show for it.
...
good, but it should come down a level, so that less experienced
technical teams can follow it too.
